...
export async function serve(options: ServeOptions = {}): Promise<DesktopControlServer> {
  ...
  return new Promise<DesktopControlServer>((resolve, reject) => {
    server.listen(port, '127.0.0.1', () => {
      console.log(`[desktop-control] Server listening on http://127.0.0.1:${port}`);
      console.log(`[desktop-control] Platform: ${process.platform} (${process.arch})`);
      console.log(`[desktop-control] Blocklist entries: ${blocklist.length}`);
      ...
      const stop = async (): Promise<void> => {
        return new Promise<void>((resolveStop) => {
          ...
          connections.clear();
          server.close(() => {
            console.log('[desktop-control] Server stopped');
            resolveStop();
          });
        });
      };
